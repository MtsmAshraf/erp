...
import { addOfferItem } from "./actions"
...
export function AddOfferItemForm({ 
  priceOfferId, 
  products, 
  customer 
}: { 
  ...
  const [state, formAction, isPending] = useActionState(addOfferItem, { error: "" })

  return (
    <div className="border-b bg-gray-50 p-4">
      <form action={formAction} className="space-y-4">
        <input type="hidden" name="priceOfferId" value={priceOfferId} />
        ...
        <div className="flex items-end gap-4">
          ...
          <div className="w-40">
            <label className="mb-1 block text-sm font-medium text-gray-700">
              Unit Price *
              <span className="ml-1 text-xs text-gray-500">(Markup: {customer.salePercentage}%)</span>
            </label>
